...
#define ESP_LCD_TOUCH_TT21100_MAX_DATA_LEN  (7+LCD_TOUCH_MAX_POINTS*10) /* 7 Header + (Points * 10 data bytes) */

static esp_err_t esp_lcd_touch_new_i2c_tt21100(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch);
...
static esp_err_t esp_lcd_touch_new_i2c_tt21100(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    esp_lcd_touch_handle_t esp_lcd_touch_tt21100 = (esp_lcd_touch_handle_t)heap_caps_calloc(1, sizeof(esp_lcd_touch_t), MALLOC_CAP_DEFAULT);
...
static esp_err_t esp_lcd_touch_tt21100_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    if (data_len > 0 && data_len < sizeof(data)) {
        ...
        if (data_len == 14) {
            ...
            /* Buttons count */
            tp->data.buttons = (LCD_TOUCH_MAX_BUTTONS < 4 ? LCD_TOUCH_MAX_BUTTONS : 4);
            ...
            tp_num = (tp_num > LCD_TOUCH_MAX_POINTS ? LCD_TOUCH_MAX_POINTS : tp_num);
